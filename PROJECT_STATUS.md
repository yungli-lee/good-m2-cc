...
- M2-004 Homepage Featured API：已完成首頁精選物件資料來源串接，使用 `/api/public/featured-properties`；限制：本階段不做 media/search/SEO/TikTok。
...
- 精選物件資料條件、API、後台 `is_featured` 切換與首頁資料串接已存在；媒體、搜尋與 SEO 仍未納入本階段。
