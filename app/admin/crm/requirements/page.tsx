...
export default async function RequirementsPage({ searchParams }: Props) {
  ...
  return <main className="section"><div className="container">
    ...
    <form className="form-grid" aria-label="客需搜尋與篩選">
      ...
      <label className="field"><span>{filters.transactionType === "rent" ? "物件月租（萬元）" : filters.transactionType === "buy" ? "物件價格（萬元）" : "物件價格／月租（萬元）"}</span><input className="input" type="number" min="0" name="propertyPrice" defaultValue={filters.propertyPrice} /></label>
