...
import { DeleteNavigationItemForm } from "@/components/admin/delete-navigation-item-form";
import { requireRole } from "@/lib/auth";
import { listAdminSitePages } from "@/lib/home-cms/queries";
import { getAdminNavigationItem } from "@/lib/navigation";
import { deleteNavigationItemAction, updateNavigationItemAction } from "../../actions";
...
export default async function EditNavigationItemPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  if (!item) notFound();
  const update = updateNavigationItemAction.bind(null, id);
  const remove = deleteNavigationItemAction.bind(null, id);
  return <main className="section"><div className="container">
    ...
    <NavigationItemForm action={update} item={item} pages={pages.filter((page) => page.status === "published" && !page.archived_at)} />
    <DeleteNavigationItemForm action={remove} label={item.label} />
  </div></main>;
}
