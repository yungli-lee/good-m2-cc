...
import { createPropertyPeopleAction, archivePropertyPeopleAction, updatePropertyPeopleAction } from "../../relation-actions";
  ...
  return (
    <main className="section">
      <div className="container">
        ...
        />
        <section className="card" style={{ marginTop: 18 }}><div className="card-body"><h2 style={{ marginTop: 0 }}>關聯客戶</h2>{query.relation_error ? <div className="notice">{query.relation_error === "duplicate" ? "此客戶與物件已存在相同的有效關係。" : "關聯操作失敗，請稍後再試。"}</div> : null}{relatedPeople?.length ? <div className="table-wrap"><table><thead><tr><th>顯示名稱</th><th>正式姓名</th><th>關係</th><th>聯絡方式</th><th>操作</th></tr></thead><tbody>{relatedPeople.map((relation) => <tr key={relation.id}><td><Link href={`/admin/people/${relation.person?.id}`}>{relation.person?.display_name || relation.person_id}</Link></td><td>{relation.person?.legal_name || "-"}</td><td>{relationshipLabels[relation.relationship_type as keyof typeof relationshipLabels]}</td><td>{relation.person?.phone || relation.person?.email || "-"}</td><td><details><summary className="button ghost">編輯</summary><form className="form-grid" action={updatePropertyPeopleAction}><input type="hidden" name="id" value={relation.id}/><input type="hidden" name="property_id" value={id}/><input type="hidden" name="person_id" value={relation.person_id}/><label className="field"><span>關係</span><select className="select" name="relationship_type" defaultValue={relation.relationship_type}>{relationshipTypes.map(t=><option key={t} value={t}>{relationshipLabels[t]}</option>)}</select></label><label className="field"><span>自訂名稱（其他）</span><input className="input" name="relationship_label" defaultValue={relation.relationship_label || ""}/></label><label className="field full"><span>備註</span><textarea className="textarea" name="note" defaultValue={relation.note || ""} maxLength={2000}/></label><label className="field"><span>開始日期</span><input className="input" type="date" name="started_at" defaultValue={relation.started_at || ""}/></label><label className="field"><span>結束日期</span><input className="input" type="date" name="ended_at" defaultValue={relation.ended_at || ""}/></label><button className="button" type="submit">儲存關係</button></form></details><form action={archivePropertyPeopleAction.bind(null,relation.id,id,relation.person_id)}><button className="button danger" type="submit">封存</button></form></td></tr>)}</tbody></table></div> : <p className="muted">尚未建立關聯客戶</p>}<h3>新增關聯客戶</h3><form className="form-grid" action={createPropertyPeopleAction}><input type="hidden" name="property_id" value={id}/><label className="field"><span>People</span><select className="select" name="person_id" required><option value="">請選擇</option>{peopleOptions?.map((p)=><option key={p.id} value={p.id}>{p.display_name} {p.phone || p.email ? `（${p.phone || p.email}）` : ""}</option>)}</select></label><label className="field"><span>關係</span><select className="select" name="relationship_type" defaultValue="contact">{relationshipTypes.map(t=><option key={t} value={t}>{relationshipLabels[t]}</option>)}</select></label><label className="field full"><span>備註</span><textarea className="textarea" name="note" maxLength={2000}/></label><button className="button" type="submit">建立關聯</button></form></div></section>
      </div>
    </main>
  );
}
