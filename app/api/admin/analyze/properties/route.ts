...
export async function GET(request: Request) {
  ...
  try {
    return response({ ok: true, data: await getAnalyticsProperties(range, getDashboardEnvironment()) });
  } catch (error) {
    console.error("[analytics_properties_failed]", { name: error instanceof Error ? error.name : "unknown", reason: error instanceof Error && error.message === "analytics_range_too_large" ? "range_too_large" : "query_failed" });
    return response({ ok: false, error: "analytics_properties_unavailable" }, 500);
  }
}
