...
export async function GET(request: Request) {
  ...
  try {
    return response({ ok: true, data: await getAnalyticsSources(range, getDashboardEnvironment()) });
  } catch (error) {
    console.error("[analytics_sources_failed]", {
      name: error instanceof Error ? error.name : "unknown",
      reason: error instanceof Error && error.message === "analytics_range_too_large" ? "range_too_large" : "query_failed"
    });
    return response({ ok: false, error: "analytics_sources_unavailable" }, 500);
  }
}
