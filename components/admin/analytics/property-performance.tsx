...
const sortOptions: Array<{ key: PropertySortKey; label: string }> = [
  ...
  { key: "viewInquiryConversionRate", label: "轉換率" }, { key: "ctaRate", label: "聯絡互動率" }
];
...
export function PropertyPerformance({ properties, error = false }: Props) {
  ...
  return <section className="analytics-property-panel" aria-labelledby="analytics-property-title">
    <div className="analytics-property-heading"><div><h2 id="analytics-property-title">物件成效</h2><p className="muted">依物件彙整瀏覽、互動與詢問；訪客及造訪次數採不重複計數。</p></div></div>
    ...
    <div className="analytics-property-desktop table-wrap"><table><thead><tr><th>物件</th><th>瀏覽</th><th>訪客</th><th>LINE</th><th>電話</th><th>詢問</th><th>轉換率</th><th>聯絡互動率</th><th>其他數據</th></tr></thead><tbody>{rows.map((row) => <tr key={row.propertyId}><td><PropertyName row={row} /></td><td>{row.views}</td><td>{row.visitors}</td><td>{row.lineClicks}</td><td>{row.phoneClicks}</td><td>{row.inquiries}</td><td>{rate(row.viewInquiryConversionRate)}</td><td>{rate(row.ctaRate)}</td><td><details><summary>展開</summary><span>造訪次數 {row.sessions} · 媒體 {row.mediaViews} · 分享 {row.shares} · 地圖 {row.mapOpens} · 開始詢問 {row.inquiryStarts}</span></details></td></tr>)}</tbody></table></div>
    <div className="analytics-property-mobile">{rows.map((row) => <article key={row.propertyId}><PropertyName row={row} /><dl><div><dt>瀏覽</dt><dd>{row.views}</dd></div><div><dt>訪客</dt><dd>{row.visitors}</dd></div><div><dt>LINE</dt><dd>{row.lineClicks}</dd></div><div><dt>電話</dt><dd>{row.phoneClicks}</dd></div><div><dt>詢問</dt><dd>{row.inquiries}</dd></div><div><dt>轉換</dt><dd>{rate(row.viewInquiryConversionRate)}</dd></div><div><dt>聯絡互動率</dt><dd>{rate(row.ctaRate)}</dd></div></dl><details><summary>其他數據</summary><p>造訪次數 {row.sessions} · 媒體 {row.mediaViews} · 分享 {row.shares} · 地圖 {row.mapOpens} · 開始詢問 {row.inquiryStarts}</p></details></article>)}</div>
  </section>;
}
