...
import { HomeSocialImageField } from "@/components/admin/home-social-image-field";
...
export function SiteDisplaySettingsForm({ settings }: { settings: SiteDisplaySettings }) {
  ...
  return <form className="form-grid" action={action}>
    ...
    <HomeSocialImageField initialUrl={settings.home_social_image_url} initialPath={settings.home_social_image_path} />
    <div className="field full"><button className="button" disabled={pending}>{pending ? "儲存中…" : "儲存／發布前台顯示設定"}</button></div>
  </form>;
}
