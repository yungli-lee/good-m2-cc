...
export function DesktopReminders({ pages }: { pages: Reminder[] }) {
  ...
  return <>
    <div className="life-note-desktop-grid">{visible.map((page) => <article className="life-note-card" key={page.id}>
      {page.media_public_url ? <DeliveredImage className="life-note-card-image" sourceUrl={page.media_public_url} tier="card" sizes="(min-width: 901px) 579px, 1px" alt={page.title} loading="lazy" /> : <div className="life-note-card-image life-note-card-placeholder" aria-hidden="true"><span>Life Notes</span></div>}
