...
export function HomePropertyCollection({ kind, properties }: { kind: "featured" | "latest"; properties: HomeProperty[] }) {
  ...
  return (
    <section className="problem-section" id={featured ? "featured-properties" : "latest-properties"}>
      ...
      {properties.length ? <div className="property-discovery"><div className="property-card-track">{properties.map((property) => <HomePropertyCard property={property} key={property.id} />)}</div><div className="property-carousel-actions"><Link className="button" href="/properties">查看更多物件</Link></div></div> : <p className="note">目前{featured ? "精選" : "公開"}物件整理中，歡迎稍後再查看。</p>}
    </section>
  );
}
