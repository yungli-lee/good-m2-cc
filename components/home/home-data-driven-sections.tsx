...
import { HomePropertySearch, type HomeProperty } from "@/components/home/home-property-search";
import { PropertyCarousel } from "@/components/home/property-carousel";
import type { ContentItem } from "@/lib/content/types";

export function HomePropertyCollection({ kind, properties, autoplay, intervalSeconds }: { kind: "featured" | "latest"; properties: HomeProperty[]; autoplay: boolean; intervalSeconds: number }) {
  ...
  return (
    <section className="problem-section" id={featured ? "featured-properties" : "latest-properties"}>
      ...
      {properties.length ? <PropertyCarousel kind={kind} properties={properties} autoplay={autoplay} intervalSeconds={intervalSeconds} /> : <p className="note">目前{featured ? "精選" : "公開"}物件整理中，歡迎稍後再查看。</p>}
    </section>
  );
}
