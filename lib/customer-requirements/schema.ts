...
const optionalQueryBoolean=z.enum(["required","not_required"]).optional();
const optionalDate=z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional();
export const requirementListQuerySchema=z.object({
 page:z.coerce.number().int().min(1).default(1),pageSize:z.coerce.number().int().min(10).max(100).default(20),search:z.string().trim().max(120).default(""),personId:z.string().uuid().optional(),requirementType:z.enum(requirementTypes).optional(),transactionType:z.enum(transactionTypes).optional(),status:z.enum(requirementStatuses).optional(),urgency:z.enum(urgencyLevels).optional(),assignedUserId:z.string().uuid().optional(),city:z.string().trim().max(80).optional(),district:z.string().trim().max(80).optional(),propertyCategory:z.enum(propertyCategories).optional(),purchaseTimeline:z.enum(purchaseTimelines).optional(),
