import { z } from "zod";
import { allowedPropertyCategories, fundingStatuses, propertyCategories, purchaseTimelines, requirementStatuses, requirementTypes, transactionTypes, urgencyLevels } from "./constants.ts";
...
}).superRefine((v,c)=>{const allowed=allowedPropertyCategories(v.requirement_type);if(v.property_categories.some(value=>!allowed.includes(value)))c.addIssue({code:"custom",path:["property_categories"],message:"物件類型與需求類型不相容"});if(!v.cities.length&&!v.districts.length&&!v.area_note)c.addIssue({code:"custom",path:["area_note"],message:"至少填寫縣市、行政區或區域備註"});const ranges:Array<[number|null|undefined,number|null|undefined,string]>=[[v.sale_budget_min,v.sale_budget_max,"sale_budget_max"],[v.rent_budget_min,v.rent_budget_max,"rent_budget_max"],[v.price_per_ping_min,v.price_per_ping_max,"price_per_ping_max"],[v.land_area_min,v.land_area_max,"land_area_max"],[v.building_area_min,v.building_area_max,"building_area_max"],[v.bedrooms_min,v.bedrooms_max,"bedrooms_max"],[v.floor_min,v.floor_max,"floor_max"]];for(const [a,b,p] of ranges)if(a!=null&&b!=null&&a>b)c.addIssue({code:"custom",path:[p],message:"上限不得小於下限"});if(v.transaction_type==="buy"){if(v.sale_budget_max==null)c.addIssue({code:"custom",path:["sale_budget_max"],message:"買賣客需必須填寫預算上限"});if(v.rent_budget_min!=null||v.rent_budget_max!=null)c.addIssue({code:"custom",path:["rent_budget_max"],message:"買賣客需不得填租金"});}else{if(v.rent_budget_max==null)c.addIssue({code:"custom",path:["rent_budget_max"],message:"租賃客需必須填寫租金上限"});if(v.sale_budget_min!=null||v.sale_budget_max!=null)c.addIssue({code:"custom",path:["sale_budget_max"],message:"租賃客需不得填買賣預算"});}});
