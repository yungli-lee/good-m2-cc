...
export async function getSiteDisplaySettings(): Promise<SiteDisplaySettings> {
  ...
  const { data, error } = await supabase.from("site_display_settings").select("featured_property_limit,featured_property_autoplay,featured_property_interval_seconds,latest_property_limit,latest_property_autoplay,latest_property_interval_seconds,knowledge_page_size,home_social_image_url,home_social_image_path").eq("id", "default").maybeSingle();
