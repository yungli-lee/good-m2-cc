...
alter type public.audit_action add value if not exists 'login_denied';
...
drop policy if exists "users update own login tracking" on public.profiles;
create policy "users update own login tracking" on public.profiles
